const operationsDoc1 = `  
  mutation insertStats($favourited: Int!, $userId: String!, $watched:Boolean!, $videodId:String!) {
    insert_stats_one(object: {favourited: $favourited, userId: $userId, videoId: $videodId, watched: $watched}) {
      favourited
      id
      userId
    }
...
export async function updateStats(token, { favourited, userId, watched, videoId }) {
  const operationsDoc = `  
  mutation updateStats($favourited: Int!, $userId: String!, $watched: Boolean!, $videodId: String!) {
    update_stats(
